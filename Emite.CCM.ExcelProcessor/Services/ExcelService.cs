...
using Emite.CCM.ExcelProcessor.CustomValidation;
using Emite.CCM.ExcelProcessor.CustomTemplate;

namespace Emite.CCM.ExcelProcessor.Services
{
        ...
        private async Task<Dictionary<string, object?>> CustomValidationPerRecordHandler(string module, Dictionary<string, object?> rowValue)
        {
            ...
            switch (module)
            {
                case nameof(AgentState):
					return await AgentValidator.ValidatePerRecordAsync(_context, rowValue);
				case nameof(CustomerState):
					return await CustomerValidator.ValidatePerRecordAsync(_context, rowValue);
				case nameof(CallState):
					return await CallValidator.ValidatePerRecordAsync(_context, rowValue);
				case nameof(TicketState):
					return await TicketValidator.ValidatePerRecordAsync(_context, rowValue);
